...
<section class="fighters">
        ...
        <div class="stats d-flex justify-content-around align-items-center">
            <div class="stats__attacker d-flex flex-column gap-3">
                <div class="health__attacker" role="progressbar" aria-valuenow="<?= $attacker->getHealth() ?>" aria-valuemin="0" aria-valuemax="<?= $attacker->getStats()['maxHealth'] ?>">
                ...
                <div class="energy__attacker" role="progressbar" aria-valuenow="<?= $attacker->getEnergy() ?>" aria-valuemin="0" aria-valuemax="<?= $attacker->getStats()['maxEnergy'] ?>">
                    <?php $energy_percentA = $attacker->getEnergy() / $attacker->getStats()['maxEnergy'] * 100; ?>
                    <div class="progress-bar bg-success progress-bar-striped progress-bar-animated" style="width: <?= $energy_percentA ?>%"><?= $attacker->getEnergy() ?></div>
                </div>
            </div>

            <div class="stats__defender d-flex flex-column gap-3">
                <div class="health__defender" role="progressbar" aria-valuenow="<?= $defender->getHealth() ?>" aria-valuemin="0" aria-valuemax="<?= $defender->getStats()['maxHealth'] ?>">
                ...
                <div class="energy__defender" role="progressbar" aria-valuenow="<?= $defender->getEnergy() ?>" aria-valuemin="0" aria-valuemax="<?= $defender->getStats()['maxEnergy'] ?>">
                    <?php $energy_percentD = $defender->getEnergy() / $defender->getStats()['maxEnergy'] * 100; ?>
                    <div class="progress-bar bg-success progress-bar-striped progress-bar-animated" style="width: <?= $energy_percentD ?>%"><?= $defender->getEnergy() ?></div>
                </div>
            </div>
        </div>
